...
function saveVisitSession() {
  ...
  if (sessionData.pages.length > 0 || sessionData.socialClicks.length > 0 || 
      sessionData.cartActions.length > 0 || sessionData.contactActions.length > 0 || 
      sessionData.checkoutActions.length > 0) {
    
    // Calculate total session duration
    sessionData.endTime = new Date().toISOString();
    sessionData.endTimestamp = Date.now();
    sessionData.totalDuration = sessionData.endTimestamp - sessionData.startTimestamp;
...
function getVisitSession() {
  return JSON.parse(localStorage.getItem(VISIT_SESSION_KEY)) || {
    startTime: new Date().toISOString(),
    startTimestamp: Date.now(),
    currentPage: 'home',
    pageStartTime: Date.now(),
...
function trackCartAction(action, data = {}) {
  ...
  const lastAction = sessionData.cartActions[sessionData.cartActions.length - 1];
  if (!lastAction || lastAction.action !== action || Date.now() - lastAction.timestamp > 1000) {
...
function summarizeSession(sessionData) {
  ...
  if (sessionData.pages.length > 0) {
    const pageSummary = sessionData.pages.map(page => 
      `${page.page} (${formatDuration(page.duration)})`
    ).join(', ');
  ...
  if (sessionData.currentPage && sessionData.totalDuration) {
    const currentPageDuration = sessionData.endTimestamp - sessionData.pageStartTime;
    summary.push(`Current Page: ${sessionData.currentPage} (${formatDuration(currentPageDuration)})`);
  }
  ...
  if (sessionData.socialClicks.length > 0) {
    const socialPlatforms = sessionData.socialClicks.map(click => click.platform);
    summary.push(`Social Clicks: ${[...new Set(socialPlatforms)].join(', ')} (${sessionData.socialClicks.length} clicks)`);
  }
  ...
  if (sessionData.cartActions.length > 0) {
    ...
    const actionSummary = Object.entries(actionCounts).map(([action, count]) => 
      `${action} (${count})`
    ).join(', ');
  ...
  if (sessionData.contactActions.length > 0) {
    const submitCount = sessionData.contactActions.filter(a => a.action === 'form_submit').length;
    const inputCount = sessionData.contactActions.filter(a => a.action === 'form_input').length;
    summary.push(`Contact Form: ${inputCount} fields filled, ${submitCount} submissions`);
  }
  
  if (sessionData.checkoutActions.length > 0) {
    const shippingChanges = sessionData.checkoutActions.filter(a => a.action === 'shipping_change').length;
    const noteTyped = sessionData.checkoutActions.filter(a => a.action === 'note_typed').length;
    const submits = sessionData.checkoutActions.filter(a => a.action === 'order_submit').length;
    
    let checkoutSummary = [];
    if (shippingChanges > 0) checkoutSummary.push(`shipping changes: ${shippingChanges}`);
...
- Contact Fields: ${Object.keys(sessionData.formFields).filter(k => k.includes('contact')).length}
      ...
      orderNote.addEventListener('blur', () => {
        if (orderNote.value.length > 5) {
          trackCheckoutAction('note_typed', { 
            ...
            preview: orderNote.value.substring(0, 30) + (orderNote.value.length > 30 ? '...' : '')
          });
        }
      });
    }
...
function setupCheckoutAlertTimer() {
  ...
  const recentAlerts = checkoutAlerts.filter(alertTime => 
    now - alertTime < TIMING.checkoutAlertCooldown
  );
