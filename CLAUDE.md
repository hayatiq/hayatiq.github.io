...
Products live only in the `PRODUCTS` array in `js/products.js` (not `js/script.js` — that file only renders them). All keys are required on every product object: `id`, `name`, `slug`, `price`, `category`, `images`, `short`, `ingredients`, `how`, `tips`, `warns`, `storage` (`subtitle`, `concerns`, `badge`, `topSelling`, `display_order` are optional; `ingredients`/`how`/`tips` may be left as `[]` when real content isn't ready yet — `renderDetail()` skips rendering a section entirely when its array is empty rather than showing an empty accordion). Notes:
